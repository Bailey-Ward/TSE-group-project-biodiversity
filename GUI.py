...
class GUI:
    ...
    #=================================================================================================================
    #Import data button
    def importDataButton(user, password, databaseName):
        fileLocation = simpledialog.askstring(title="File directory:",prompt="Enter the file directory:")
        btn = Button(window, text="Import", bg = '#778beb', highlightbackground="Black", command=database.insertData(fileLocation, user, password, databaseName))
        btnFont = ("Agency FB", 20, "bold")
        btn.config(width=40)
        btn.config(font = btnFont)
        btn.grid(column=0, row=0)
        btn.place(x=1470, y=240)
    ...
    def displayDataButton(username,databaseName,password):
        btn = Button(window, text="Read from DB", bg = '#778beb', highlightbackground="Black", command=database.displayData(username,databaseName,password))
        btnFont = ("Agency FB", 20, "bold")
        btn.config(width=40)
        btn.config(font = btnFont)
        btn.grid(column=0, row=0)
        btn.place(x=1470, y=340)

    #=================================================================================================================
    #Delete from database button
    def deleteDataButton(siteID, user, password, databaseName):
        btn = Button(window, text="Remove site from DB", bg = '#778beb', highlightbackground="Black", command=database.removeData(siteID, user, password, databaseName))
        btnFont = ("Agency FB", 20, "bold")
        btn.config(width=40)
        btn.config(font = btnFont)
        btn.grid(column=0, row=0)
